...
def ablationZone(gl_workspace, glacier, gl_area, name, debris, n_deb_pixels_cut_ablationZone, x_val, halfTribWidthRemove, hole_fill):
    ...
    if arcpy.management.GetCount("glacier_debris.shp")[0] == "0":
        ...
        if arcpy.management.GetCount(ablationZoneName_beforeSimplify)[0] == "0":
            ...
            arcpy.Buffer_analysis(ablationZoneName_beforeSimplify, 'ablZone_ShapeIN.shp', -glacier_width*halfTribWidthRemove, '', '', 'NONE')
            arcpy.MultipartToSinglepart_management('ablZone_ShapeIN.shp','ablZone_ShapeIN_explode.shp')
            shapeSizes = [] # find biggest shape and delete all others, this cuts tributaries 
            with arcpy.da.SearchCursor('ablZone_ShapeIN_explode.shp', ['SHAPE@AREA']) as cursor:
                for row in cursor:
                    shapeSizes.append(row[0])
            with arcpy.da.UpdateCursor('ablZone_ShapeIN_explode.shp', ['SHAPE@AREA']) as cursor:
                for row in cursor:
                    if row[0] != max(shapeSizes):
                        cursor.deleteRow()
            arcpy.Buffer_analysis('ablZone_ShapeIN_explode.shp', 'ablZone_ShapeOUT.shp', (glacier_width*halfTribWidthRemove)+5, '', '', 'NONE') #plus 5m then cut out again, just avoiding sliver polygons
            arcpy.Intersect_analysis(['ablZone_ShapeOUT.shp',ablationZoneName_beforeSimplify], ablationZoneName_withoutSmallTribs, "ALL")
